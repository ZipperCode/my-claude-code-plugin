---
description: "Multi-model consultation: parallel codex + gemini analysis with synthesis"
argument-hint: "<question> [--codex-only | --gemini-only | --backend | --frontend]"
allowed-tools:
  - Read
  - Glob
  - Grep
...
1. Run: `bash "$CLAUDE_PROJECT_DIR"/.claude/hooks/maestro/check-deps.sh`
...
**Note**: gemini's `sandbox` is a **boolean** (`false`), NOT a string. Gemini does NOT support `cd` parameter — do not include it.
...
3. **Extract summary**: Invoke maestro-context-curator agent to read the consultation file and produce a structured summary (length controlled by policy)
...
If this consultation involved substantial analysis (both models consulted, or detailed single-model analysis), invoke the `maestro-learning-extractor` agent to capture project-specific conventions, decisions, and patterns discovered during the consultation.
