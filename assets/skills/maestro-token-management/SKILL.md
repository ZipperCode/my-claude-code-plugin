---
name: maestro-token-management
description: "Token budget management, summary strategies, and storage lifecycle"
...
New: prompt(guided, no hard limit) → MCP outputs full analysis → save original to .maestro/consultations/
                                                                → maestro-context-curator extracts summary
...
- Cleanup is triggered by maestro-context-curator at stage transitions (keyDecisions) and after MCP calls (consultations)
...
- **Consumer-side extraction**: After MCP response, maestro-context-curator extracts only key decisions and actionable items
...
- **Trigger**: At the end of each workflow stage, invoke maestro-context-curator agent
- **Two-tier summaries**:
...
2. **Compress existing context** — invoke maestro-context-curator immediately
...
- **Detection agents** (maestro-workflow-detector): haiku model, ≤300 word responses
- **Quality agents** (maestro-quality-gate): sonnet model, ≤500 word responses
- **Coordinator agents** (maestro-model-coordinator): default model, structured output only
- **Curator agents** (maestro-context-curator): haiku model, summary-focused (length per policy)
