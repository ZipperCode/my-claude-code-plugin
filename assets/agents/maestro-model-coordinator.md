---
name: maestro-model-coordinator
...
- "maestro-role-prompts" — Stage-aware role prompt templates for codex/gemini
- "maestro-mcp-protocols" — MCP invocation conventions and parameter differences
...
Instead of using fixed prompts, **select role templates from the "maestro-role-prompts" skill** based on the current workflow stage:
...
2. **Retrieve the matching role template** from "maestro-role-prompts" skill for both codex and gemini
...
2. **Extract summaries**: Use maestro-context-curator to produce structured summaries (length per policy)
